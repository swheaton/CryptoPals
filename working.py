...
ScoreText = namedtuple('ScoreText','score text decodeByte')
charFreqList = [8.167, 1.492, 2.782, 4.253, 12.702, 2.228, 2.015, 6.094, 6.996, 0.153, 0.772, 4.025, 2.406, 6.749,
...
def singleXorDecrypt(hexVal, useNonPrintable=True):
    bestScore = ScoreText(score=sys.float_info.max, text = '',decodeByte='')
    ...
    for ind in xrange(256):
        ...
        if(score < bestScore.score):
            bestScore = ScoreText(score, result.decode('hex'), decodeByte=chr(ind))
...
def findSingleXorCipherInFile(fileName):
    with open(fileName) as file:
        bestPair = ScoreText(score=sys.float_info.max, text = '')
        for line in file:
